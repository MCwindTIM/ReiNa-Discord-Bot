  ...
  const serverQueue = queue.get(message.guild.id);
	...
	if (cmd === `${prefix}db`){
		...
		if (!messageArray[1]){
			const embed = new Discord.RichEmbed()
				embed
			.setDescription(`${message.author}` + " Senpai, 現在的db是:" + `**${serverQueue.volume}**`)
			.setColor(0xcc0000)
			.setTitle('ReiNa Bot')
			.setURL("https://mcwind.tk")
			.setTimestamp()
		...
		const embed = new Discord.RichEmbed()
			embed
		.setDescription(`${message.author}` + " 是的Senpai, 我把db調整到: " + `**${messageArray[1]}**` + "了哦!")
		.setColor(0xcc0000)
		.setTitle('ReiNa Bot')
		.setURL("https://mcwind.tk")
		.setTimestamp()
